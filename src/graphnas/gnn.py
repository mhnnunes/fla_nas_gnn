...
from graphnas.utils.model_constants import act_map


class GraphNet(torch.nn.Module):

    def __init__(self, actions, num_feat, num_label,
                 drop_out=0.6, multi_label=False,
        ...
        # layer module
        self.build_model(actions, batch_normal, drop_out,
                         num_feat, num_label, state_num)

    def build_model(self, actions, batch_normal, drop_out,
                    num_feat, num_label, state_num):
        self.layers = torch.nn.ModuleList()
        self.gates = torch.nn.ModuleList()
        self.prediction = None
        # self.build_hidden_layers(actions, batch_normal,
        #                          drop_out, self.layer_nums,
        #                          num_feat, num_label, state_num)
    ...
    def build_hidden_layers(self, actions, batch_normal, drop_out,
                            layer_nums, num_feat, num_label, state_num=6):
        ...
        for i in range(layer_nums):
            ...
            self.layers.append(
                NASLayer(attention_type, aggregator_type,
                         act, head_num, in_channels,
                         out_channels, dropout=drop_out,
                         concat=concat, residual=residual,
    ...
    def get_param_dict(self, old_param=None, update_all=True):
        ...
        for i in range(self.layer_nums):
            ...
            if key in result:
                new_param = NASLayer.merge_param(result[key],
                                                 new_param,
                                                 update_all)
...
class NASLayer(nn.Module):
    def __init__(self, attention_type,
                 aggregator_type, act, head_num,
                 in_channels, out_channels=8, concat=True,
                 dropout=0.6, pooling_dim=128,
        ...
        for hid in range(self.num_heads):
            ...
            self.prp.append(AttentionPrepare(in_channels,
                                             out_channels,
                                             self.attention_dim,
                                             dropout))
            agg = NASLayer.aggregator_map(aggregator_type,
                                          out_channels,
                                          pooling_dim)
            self.agg.append(agg)
            self.red.append(NASLayer.attention_map(attention_type,
                                                   dropout,
                                                   agg,
                                                   self.attention_dim))
    ...
    def attention_map(attention_type, attn_drop, aggregator, attention_dim):
        if attention_type == "gat":
            ...
            return GeneralizedLinearReduce(attn_drop, attention_dim,
                                           aggregator)
    ...
    def get_param_dict(self):
        ...
        key = "%d_%d_%d_%s" % (self.in_channels,
                               self.out_channels,
                               self.num_heads,
                               self.attention_type)
        prp_key = "_".join([key,
                            str(self.attention_dim),
                            "prp"])
        agg_key = "_".join([key,
                            str(self.pooling_dim),
    ...
    def forward(self, features, g):
        ...
        if not self.concat:
            ...
            output = torch.cat(
                [g.pop_n_repr('head%d' % hid)
                 for hid in range(self.num_heads)], dim=1)
        del last
        return output


'''
    The whole process of each layer of GNN includes
    but is not limited following action：
...
class MeanPoolingAggregator(MaxPoolingAggregator):
    ...
    def __init__(self, input_dim,
                 pooling_dim=512, num_fc=1,
                 act=F.leaky_relu_):
        super(MeanPoolingAggregator, self).__init__(input_dim,
                                                    pooling_dim,
                                                    num_fc,
                                                    act)
...
class MLPAggregator(MaxPoolingAggregator):
    ...
    def __init__(self, input_dim,
                 pooling_dim=512, num_fc=1,
                 act=F.leaky_relu_):
        super(MLPAggregator, self).__init__(input_dim,
                                            pooling_dim,
                                            num_fc,
                                            act)
...
class LSTMAggregator(SumAggregator):
    ...
    def __init__(self, input_dim, pooling_dim=512):
        ...
        self.lstm = nn.LSTM(input_dim, pooling_dim,
                            batch_first=True,
                            bias=False)
...
class GRUAggregator(SumAggregator):
    ...
    def __init__(self, input_dim, pooling_dim=512):
        super(LSTMAggregator, self).__init__()
        self.lstm = nn.GRU(input_dim, pooling_dim,
                           batch_first=True,
                           bias=False)
        self.linear = nn.Linear(pooling_dim, input_dim)
    ...
    def __init__(self, headid, indim, hiddendim, activation, residual):
        ...
        if residual:
            if indim != hiddendim:
                ...
                nn.init.xavier_normal_(self.residual_fc.weight.data,
                                       gain=1.414)
