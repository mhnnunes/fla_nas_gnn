...
from graphnas_variants.macro.pyg_gnn_layer import GeoLayer
...
class GraphNet(BaseNet):
    ...
    def __init__(self, actions, num_feat, num_label,
                 ...
                 batch_normal=True, state_num=5, residual=False):
        self.residual = residual
        self.batch_normal = batch_normal
        super(GraphNet, self).__init__(actions,
                                       num_feat,
                                       num_label,
                                       drop_out,
                                       multi_label,
    ...
    def build_model(self, actions, batch_normal, drop_out, num_feat,
                    num_label, state_num):
        if self.residual:
        ...
        self.build_hidden_layers(actions, batch_normal, drop_out,
                                 self.layer_nums, num_feat,
                                 num_label, state_num)
        ...
        for i in range(layer_nums):
            ...
            if self.batch_normal:
                self.bns.append(torch.nn.BatchNorm1d(in_channels,
                                                     momentum=0.5))
            self.layers.append(
                GeoLayer(in_channels, out_channels, head_num, concat,
            ...
            if self.residual:
                if concat:
                    self.fcs.append(torch.nn.Linear(in_channels,
    ...
    def forward(self, x, edge_index_all):
        # print('entrou na forward do macro pyg_gnn')
        output = x
        if self.residual:
            for i, (act, layer, fc) in enumerate(zip(self.acts,
                                                     self.layers,
                                                     self.fcs)):
    ...
    def get_param_dict(self, old_param=None, update_all=True):
        ...
        for i in range(self.layer_nums):
            ...
            if key in result:
                new_param = self.merge_param(result[key],
                                             new_param, update_all)
                result[key] = new_param
            else:
                result[key] = new_param
        if self.residual:
            for i, fc in enumerate(self.fcs):
                key = "layer_" + i + "_fc_" + fc.weight.size(0)
                key += "_" + fc.weight.size(1)
    ...
    def load_param(self, param):
        ...
        if self.residual:
            for i, fc in enumerate(self.fcs):
                key = "layer_" + i + "_fc_"
                key += fc.weight.size(0) + "_" + fc.weight.size(1)
                if key in param:
