...
def batch_agg_map(agg_type, in_channels, hidden_dim, num_head, dropout):
    if agg_type in ["add", "sum", "mean", "max"]:
        return BatchBasicAggregator(agg_type=agg_type,
                                    num_head=num_head,
                                    dropout=dropout)
    elif "pooling" in agg_type:
        agg_type = agg_type.split("_")[0]
        return BatchPoolingAggregator(agg_type=agg_type,
                                      in_channels=in_channels,
                                      hidden_dim=hidden_dim,
                                      num_head=num_head,
                                      dropout=dropout)
    elif agg_type in ["lstm"]:
        return LSTMAggregator(in_channels,
                              hidden_dim,
                              num_head,
                              num_hidden_layers=1,
                              dropout=dropout)
        ...
        raise RuntimeError("wrong aggregate type:" + str(agg_type))
...
class ConstAttention(nn.Module):
    ...
    def forward(self, neighbor_vecs, self_vecs):
        # return torch.ones_like(neighbor_vecs).mean(
        #     dim=-1).to(neighbor_vecs.device)
...
class GatAttention(ConstAttention):
    def __init__(self, num_heads, out_channels):
        ...
        self.att_self_weight = Parameter(torch.Tensor(1,
                                                      self.num_heads,
                                                      self.out_channels))
        self.att_neighbor_weight = Parameter(torch.Tensor(1,
                                                          self.num_heads,
                                                          self.out_channels))
        # self.att_neighbor_weight = Parameter(self.in_channels,
        #                                      self.out_channels)
    ...
    def forward(self, neighbor_vecs, self_vecs):
        ...
        nigh_weight = (neighbor_vecs * self.att_neighbor_weight).sum(
            dim=-1)
        alpha = (self_vecs * self.att_self_weight).sum(dim=-1) + nigh_weight
...
class GatSymAttention(GatAttention):
    ...
    def forward(self, neighbor_vecs, self_vecs):
        neight_weight = (neighbor_vecs * self.att_neighbor_weight).sum(dim=-1)
        alpha = (self_vecs * self.att_self_weight).sum(dim=-1) + neight_weight
        alpha = F.leaky_relu(alpha, negative_slope=0.2)
        alpha_2 = \
            (neighbor_vecs * self.att_self_weight).sum(dim=-1) + \
...
class CosAttention(GatAttention):
    def forward(self, neighbor_vecs, self_vecs):
        weigh_vecs = self_vecs * self.att_self_weight
        alpha = neighbor_vecs * self.att_neighbor_weight * weigh_vecs
...
class CommonAttention(ConstAttention):
    def __init__(self, num_heads, out_channels, hidden_dim, act=lambda x: x):
        ...
        for i in range(num_heads):
            ...
            self.att_neighbor_weight.append(nn.Linear(out_channels,
                                                      hidden_dim))
    ...
    def forward(self, neighbor_vecs, self_vecs):
        ...
        for i in range(self.num_heads):
            ...
            alpha.append(
                self.general_layer(self.act(tmp_self_vecs + tmp_neight_vecs)))
    ...
    def preprocess(self, alpha, edge_index, neighbor_vecs, num_nodes):
        if isinstance(alpha, int):
            if self.training and self.dropout > 0:
                neighbor_vecs = F.dropout(neighbor_vecs,
                                          p=self.dropout,
...
class PoolingAggregator(BasicAggregator):
    def __init__(self, in_channels, hidden_dim=128,
                 agg_type="add", num_head=1, dropout=0.6):
    ...
    def forward(self, neighbor_vecs, alpha, edge_index, num_nodes):
        ...
        out = scatter_(self.agg_type, neighbor, edge_index[0],
                       dim_size=num_nodes)
        return out
...
class BatchPoolingAggregator(BatchBasicAggregator):
    def __init__(self, in_channels, hidden_dim=128,
                 num_head=1, dropout=0.6, agg_type="add"):
        super(BatchPoolingAggregator, self).__init__(agg_type,
                                                     num_head,
                                                     dropout)
        self.layers = nn.ModuleList()
        self.layers.append(nn.Linear(in_channels, hidden_dim))
...
class BatchMLPAggregator(BatchPoolingAggregator):
    def __init__(self, in_channels, hidden_dim=128, num_head=1, dropout=0.6):
        super(BatchMLPAggregator, self).__init__(in_channels,
                                                 hidden_dim,
                                                 num_head,
                                                 dropout,
                                                 "add")
...
class LSTMAggregator(BatchBasicAggregator):
    ...
    def __init__(self, in_channels, hidden_dim=128,
                 num_head=1, num_hidden_layers=1, dropout=0.6):
        super(LSTMAggregator, self).__init__(num_head=num_head,
                                             dropout=dropout)
        self.in_channels = in_channels
        self.hidden_dim = hidden_dim
        self.dropout = dropout
        ...
        assert self.agg_type in ['add', 'mean', 'max']
        self.lstm = nn.LSTM(in_channels * self.num_head,
    ...
    def forward(self, neighbor_vecs, alpha, num_sample, need_softmax=False):
        ...
        h0 = torch.zeros(self.num_hidden_layers,
                         batch_size,
                         self.hidden_dim * self.num_head
                         ).to(neighbor_vecs.device)
        c0 = torch.zeros(self.num_hidden_layers,
                         batch_size,
                         self.hidden_dim * self.num_head
                         ).to(neighbor_vecs.device)
