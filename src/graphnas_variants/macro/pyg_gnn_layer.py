...
from graphnas_variants.macro.message_passing import MessagePassing
...
class GeoLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        if agg_type in ["sum", "mlp"]:
            super(GeoLayer, self).__init__('sum')
        ...
        if self.att_type in ["generalized_linear"]:
            self.general_att_layer = torch.nn.Linear(out_channels,
                                                     1,
                                                     bias=False)
        ...
        if pool_dim != 0:
            ...
            self.pool_layer.append(torch.nn.Linear(self.out_channels,
                                                   self.pool_dim))
            self.pool_layer.append(torch.nn.Linear(self.pool_dim,
                                                   self.out_channels))
        else:
            pass
