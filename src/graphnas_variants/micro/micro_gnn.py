...
class MicroGNN(nn.Module):
    def __init__(self, action, num_feat, num_classes,
                 num_hidden, dropout=0.6, layers=2,
                 stem_multiplier=2, bias=True):
        super(MicroGNN, self).__init__()
        self._layers = layers
        self.dropout = dropout
        his_dim, cur_dim, hidden_dim, out_dim = \
            num_feat, num_feat, num_hidden, num_hidden
        self.cells = nn.ModuleList()
        for i in range(layers):
            cell = Cell(action, his_dim, cur_dim,
                        hidden_dim, out_dim,
                        concat=False, bias=bias)
            self.cells += [cell]
            his_dim = cur_dim
            if action[-1] == "concat":
                cur_dim = cell.multiplier * out_dim
            else:
                cur_dim = out_dim
...
class Cell(nn.Module):
    ...
    def __init__(self, action_list, his_dim,
                 cur_dim, hidden_dim, out_dim,
                 concat, bias=True):
        '''

        :param action_list:
        like ['self_index', 'gnn'] * n +['act', 'concat_type']
        :param his_dim:
        :param cur_dim:
        :param hidden_dim:
        :param out_dim:
        ...
        :param bias:
        '''
        # current version only support this situation
    ...
    def _compile(self, action_list):
        ...
        for i, action in enumerate(cells_info):
            if i % 2 == 0:
                ...
                self._gnn.append(gnn_map(action,
                                         self.hidden_dim,
                                         self.out_dim,
                                         self.concat_of_multihead,
                                         self.bias))
